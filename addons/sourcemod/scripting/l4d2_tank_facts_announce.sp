...
public void PrintTankSkill()
{
	...
	if (duration > 60)
	{
		Format(buffer, sizeof(buffer), "%t", "min_duration", duration / 60, duration % 60);
	}
	else
	{
		Format(buffer, sizeof(buffer), "%t", "s_duration", duration > 0 ? duration : 0);
	}
	...
	Format(info, sizeof(info), "%t", "fact_title", name);
	dp.WriteString(info);
	Format(info, sizeof(info), "%t", "fact_line1", g_eTankAttack.Punch, g_eTankAttack.Rock, g_eTankAttack.Hittable);
	dp.WriteString(info);
	Format(info, sizeof(info), "%t", "fact_line2", g_eTankResult.Incap, g_eTankResult.Death);
	dp.WriteString(info);
	Format(info, sizeof(info), "%t", "fact_line3", buffer, g_eTankResult.TotalDamage);
	dp.WriteString(info);
}
